...
static void Load() {
  text = ReadFile("data/end-scroll.txt", MEMF_PUBLIC);
  font = LoadILBMCustom("data/end-scroll-font.ilbm", BM_LOAD_PALETTE);
}
...
static void Init() {
  ...
  scroll = &recycled;
  InitSharedBitmap(scroll, WIDTH, HEIGHT + 16, 1, screen[0]);
...
static void SetupLinePointers() {
  ...
  WORD y = (LONG)(frameFromStart / 2 + 8) % (WORD)scroll->height;
...
EffectT EndScroll = { Load, UnLoad, Init, Kill, Render};
